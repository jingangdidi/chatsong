...
pub async fn start_audio_capture(tx: Sender<Vec<f32>>, mut rx_start: Receiver<(bool, oneshot::Sender<()>)>) -> Result<(cpal::Stream, u32, u16), MyError> {
    ...
    let stream = match config.sample_format() {
        ...
        sample_format => return Err(MyError::OtherError{info: format!("Unsupported sample format {}", sample_format)}),
    };

    // 4. start stream
    stream.play().map_err(|e| MyError::PlayInputStreamError{error: e})?;
