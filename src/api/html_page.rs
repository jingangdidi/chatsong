...
"###;
    result += &format!("        <button onclick='toggle()' id='left-toggle' class='left-bottom' title='{}'>
        ...
        <div id='left-insert-msg' class='left-bottom' title='{}'>
            <img src='{}' id='insert-msg' aria-hidden='true' />
        </div>
...
"###;
    ...
    // 监听点击开启goal模式按钮，开启之后再点击则无效，goal模式运行完会自动切回关闭状态
    document.getElementById('left-goal').addEventListener('click', function(event) {{
        fetch('http://{}:{}{}/goal').catch(error => {{ // 开启 goal 模式
            console.error('Failed start goal mode:', error);
        }});
        goal_mode = !goal_mode;
    ...
    // 监听点击开启朗读模式按钮
    document.getElementById('left-speaker').addEventListener('click', function(event) {{
        fetch('http://{}:{}{}/speaker')
            .then(response => {{
                if (!response.ok) {{
                    console.error(`HTTP error! status: ${{response.status}}`);
                }}
                return response.json();  // 解析 JSON 布尔值
            }})
            .then(data => {{
                if (data) {{ // 只允许本地用户使用朗读模式
                    // 本地请求成功，切换喇叭图标
                    ...
                    console.log('Speaker mode not allowed');
                }}
            }})
            .catch(error => {{
                console.error('Failed start speaker mode:', error);
            }});
    }})
    // 记忆
    ...
    }})", page_data.microphone[2], ICON_MICROPHONE1, page_data.microphone[1], ICON_MICROPHONE0, page_data.microphone[0], PARAS.addr_str, PARAS.port, v, PARAS.addr_str, PARAS.port, v, ICON_GOAL1, ICON_GOAL0, PARAS.addr_str, PARAS.port, v, ICON_SPEAKER1, ICON_SPEAKER0, PARAS.addr_str, PARAS.port, v);
    result += r###"
    // 监听点击关闭语音模式按钮
...
`;*/
    ...
    function sendNewMsgToBackend(msg) {{
        fetch('http://{}:{}{}/instruction?msg='+msg).catch(error => {{
            console.error('Failed send new instruction to server:', error);
        }});
    }}
    //showApprovalWindow('是否允许运行该工具？', true);", PARAS.addr_str, PARAS.port, v, PARAS.addr_str, PARAS.port, v);
    result += r###"
    // 提交跳过后的新指示
...
"###;
    result += &format!("                        microphoneImg.src = '{}';
                        microphoneDiv.title = '{}';
                        if (jsonData.close_goal && goal_mode) {{
                            // 更新 goal 模式图标
                            const goalImg = document.getElementById('goal');
                            goalImg.src = '{}';
                            goal_mode = false;
                        }}", ICON_MICROPHONE0, page_data.microphone[0], ICON_GOAL0);
    result += r###"
                        if (autoScroll) {
                            scroll();
                        }
