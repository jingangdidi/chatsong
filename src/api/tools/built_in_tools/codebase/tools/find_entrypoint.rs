...
/// params for find entrypoint: an error message, a function name, an API endpoint, a log line.
#[derive(Deserialize)]
struct Params {
    path: String,
    query: String,
...
impl FindEntrypoint {
    ...
    /// find entrypoint
    fn find_entrypoint(&self, path: &str, query: &str, limit: usize, only_signature: bool) -> Result<String, MyError> {
        let project = Project::load_or_create_project(path)?;
        project.search_symbols(query, limit, only_signature)
    }
}

impl BuiltIn for FindEntrypoint {
    /// get tool name
    fn name(&self) -> String {
        "find_entrypoint".to_string()
    }
    ...
    fn run(&self, args: &str) -> Result<(String, Option<String>), MyError> {
        ...
        Ok((self.find_entrypoint(&params.path, &params.query, params.limit.unwrap_or(usize::MAX), params.only_signature)?, None))
    }
